...
namespace BusTrack.BusTrack.API.ServicesAPI
{
    public class DriverServiceAPI : IDriverServiceAPI
    {
        ...
        {
            var drivers =
                await _driverRepository.GetAllDriversAsync();

            return _mapper.Map<IEnumerable<DriverDTOAPI>>(
                drivers);
        }
        ...
        public async Task<DriverDTOAPI?> GetDriverById(
            string id)
        {
            var driver =
        ...
        public async Task<DriverDTOAPI> CreateDriver(
            DriverDTOAPI driver)
        {
            var driverModel =
                _mapper.Map<DriverModelAPI>(
                    driver);

            var driverDB =
                _mapper.Map<DriverDB>(
                    driverModel);

            await _driverRepository.AddDriverAsync(
                driverDB);
        ...
        public async Task<DriverDTOAPI> UpdateDriver(
            string id,
            DriverDTOAPI driver)
        {
            var driverDB =
        ...
        {
            var filter =
                Builders<DriverDB>.Filter.Eq(
                    "Id",
                    id);

            var nameUpdate =
                new DriverNameUpdater
                {
                    Name = driver.Name
                };

            var update =
                Builders<DriverDB>.Update.Set(
                    p => p.Name,
                    nameUpdate.Name);

            var updateResult =
                await _driversCollection.UpdateOneAsync(
                    filter,
                    update);
            ...
            {
                throw new Exception(
                    $"Driver with ID {id} not found or not updated.");
            }
        }
